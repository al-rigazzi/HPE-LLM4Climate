...
class MERRA2DatasetProcessor:
    ...
    MERRA2_COLLECTIONS = {
        ...
        "M2I1NXASM": {
            ...
            "variables": ["PS", "QV2M", "SLP", "T2M", "TQI", "TQL", "TQV", "U10M", "V10M", "FRLAND", "FROCEAN", "PHIS"],
            "temporal_resolution": "1H",
    ...
    def download_data(self,
                     start_date: str,
                     end_date: str,
                     temporal_resolution: str = "3H") -> Dict[str, List[Path]]:
        """
    ...
    def _extract_variables(self,
                          file_path: Path,
                          variables: List[str],
                          pressure_levels: Optional[List[float]] = None) -> xr.Dataset:
        """
    ...
    def process_collection(self,
                          ...
                          target_variables: List[str]) -> Optional[xr.Dataset]:
        """
    ...
    def align_and_combine_datasets(self,
                                  datasets: Dict[str, xr.Dataset],
                                  temporal_resolution: str = "3H") -> xr.Dataset:
        """
        ...
        for name, ds in datasets.items():
            ...
            if not (np.array_equal(ds.lat.values, ref_lat) and
                   np.array_equal(ds.lon.values, ref_lon)):
    ...
    def create_prithvi_dataset(self,
                              combined_dataset: xr.Dataset) -> Dict[str, np.ndarray]:
        """
        ...
        for var in self.SURFACE_VARS:
            if var in combined_dataset.data_vars:
                ...
                if data.ndim == 3:
                    ...
                    logger.warning(f"Unexpected dimensions for surface variable {var}: {data.shape}")
        ...
        for var in self.VERTICAL_VARS:
            if var in combined_dataset.data_vars:
                ...
                if data.ndim == 4:
                    ...
                    logger.warning(f"Unexpected dimensions for vertical variable {var}: {data.shape}")

        # Stack arrays
        surface_array = np.stack(surface_data, axis=1) if surface_data else None  # (time, var, lat, lon)
        static_array = np.stack(static_data, axis=0) if static_data else None     # (var, lat, lon)
        vertical_array = np.stack(vertical_data, axis=1) if vertical_data else None  # (time, var, level, lat, lon)
        ...
        return {
            ...
            'static_vars': [var for var in self.STATIC_SURFACE_VARS if var in combined_dataset.data_vars],
            'vertical_vars': [var for var in self.VERTICAL_VARS if var in combined_dataset.data_vars],
        }
...
Examples:
    ...
    parser.add_argument(
        "--clean_cache",
        action="store_true",
        help="Remove cache directory after processing"
    )
