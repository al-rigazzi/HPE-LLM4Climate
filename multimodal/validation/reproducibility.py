...
def validate_rmse(rmse: xr.Dataset, reference_rmse: xr.Dataset, threshold: float = 5e-3) -> bool:
    ...
    for c in ["T2M", "U10M"]:
        rmse_validation[f"{c} dev."] = (
            rmse_validation[f"{c} local"] - rmse_validation[f"{c} ref."]
        ) / rmse_validation[f"{c} ref."]

    with pd.option_context("display.max_rows", None):
        print(rmse_validation)

    below_threshold = (rmse_validation[[f"{c} dev." for c in ["T2M", "U10M"]]] < threshold).all(
        axis=None
    )
