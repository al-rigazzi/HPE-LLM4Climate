#!/usr/bin/env python3
"""
Inference script for trained multimodal climate-text fusion models.

This script loads a trained model and performs inference on climate data and text queries.
"""
...
class MultimodalInference:
    ...
    def _load_model(self) -> ClimateTextFusion:
        ...
        model = ClimateTextFusion(
            prithvi_encoder_path=self.config["model"]["prithvi_encoder_path"],
            llama_model_name=self.config["model"]["llama_model_name"],
            freeze_prithvi=self.config["model"].get("freeze_prithvi", True),
            fusion_mode=self.config["model"].get("fusion_mode", "cross_attention"),
...
def main():
    ...
    parser.add_argument("--climate_data", type=str,
                       help="Path to climate data file (.pt)")
    parser.add_argument("--query", type=str,
                       help="Text query for analysis")
    parser.add_argument("--location", type=str,
                       help="Location name for analysis")
    parser.add_argument("--analysis_type", type=str, default="general",
