#!/usr/bin/env python3
"""
DeepSpeed-based training script for Climate-Text Fusion Model

This script trains the ClimateTextFusion model using DeepSpeed for distributed training
...
class MultimodalTrainer:
    ...
    def __init__(self, config: Dict):
        ...
        # Initialize model
        self.model = ClimateTextFusion(
            prithvi_encoder_path=config["model"]["prithvi_encoder_path"],
            llama_model_name=config["model"]["llama_model_name"],
            freeze_prithvi=config["model"].get("freeze_prithvi", True),
            fusion_mode=config["model"].get("fusion_mode", "cross_attention"),
            num_layers=config["model"].get("num_fusion_layers", 4),
