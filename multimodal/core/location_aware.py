...
GEOGRAPHIC_DATABASE = {
    # Countries
    "sweden": {
        "bounds": {"lat_min": 55.3, "lat_max": 69.1, "lon_min": 11.0, "lon_max": 24.2},
        "type": "country",
    },
    "arizona": {
        "bounds": {
    ...
    "norway": {
        "bounds": {"lat_min": 57.9, "lat_max": 71.2, "lon_min": 4.6, "lon_max": 31.3},
        "type": "country",
    },
    "finland": {
        "bounds": {"lat_min": 59.8, "lat_max": 70.1, "lon_min": 19.1, "lon_max": 31.6},
        "type": "country",
    },
    "denmark": {
        "bounds": {"lat_min": 54.5, "lat_max": 57.8, "lon_min": 8.0, "lon_max": 15.2},
        "type": "country",
    },
    "united kingdom": {
        "bounds": {"lat_min": 49.9, "lat_max": 60.9, "lon_min": -8.6, "lon_max": 1.8},
        "type": "country",
    },
    "germany": {
    ...
    "spain": {
        "bounds": {"lat_min": 27.6, "lat_max": 43.8, "lon_min": -18.2, "lon_max": 4.3},
        "type": "country",
    },
    "italy": {
        "bounds": {"lat_min": 35.5, "lat_max": 47.1, "lon_min": 6.6, "lon_max": 18.5},
        "type": "country",
    },
    "brazil": {
        "bounds": {
    ...
    "india": {
        "bounds": {"lat_min": 8.1, "lat_max": 37.1, "lon_min": 68.2, "lon_max": 97.4},
        "type": "country",
    },
    "china": {
        "bounds": {"lat_min": 18.2, "lat_max": 53.6, "lon_min": 73.6, "lon_max": 135.0},
        "type": "country",
    },
    "canada": {
        "bounds": {
    ...
    # Regions
    "scandinavia": {
        "bounds": {"lat_min": 54.5, "lat_max": 71.2, "lon_min": 4.6, "lon_max": 31.6},
        "type": "region",
    },
    "mediterranean": {
        "bounds": {"lat_min": 30.2, "lat_max": 46.3, "lon_min": -6.0, "lon_max": 42.3},
        "type": "region",
    },
    "sahel": {
        "bounds": {"lat_min": 10.0, "lat_max": 20.0, "lon_min": -20.0, "lon_max": 40.0},
        "type": "region",
    },
    "amazon": {
        "bounds": {
    ...
    "sahara": {
        "bounds": {"lat_min": 15.0, "lat_max": 30.0, "lon_min": -17.0, "lon_max": 51.0},
        "type": "region",
    },
    "himalaya": {
        "bounds": {"lat_min": 27.0, "lat_max": 35.0, "lon_min": 72.0, "lon_max": 95.0},
        "type": "region",
    },
}
...
class GeographicResolver:
    ...
    def _initialize_backend(self):
        ...
        if self.backend == "geopy":
            ...
            if not REQUESTS_AVAILABLE or not self.geonames_username:
                warnings.warn(
                    "GeoNames requires requests and username, falling back to local database"
                )
                self.backend = "local"
                return
    ...
    def _resolve_with_geopy(self, location_text: str) -> Optional[GeographicLocation]:
        ...
        try:
            ...
            location = self.geocoder.geocode(
                location_text, exactly_one=True, addressdetails=True, extratags=True
            )
    ...
    def _determine_type_from_geopy(self, raw_data: dict) -> str:
        ...
        # Check for country
        if address.get("country") and not any(
            k in address for k in ["state", "city", "town", "village"]
        ):
            return "country"
...
class SpatialCropper:
    ...
    def _create_mask_region(
        ...
        if mask_type == "binary":
            ...
            lon_center = (lon_min_idx + lon_max_idx) / 2
            lat_radius = max(
                1.0, (lat_max_idx - lat_min_idx) / 2 * focus_strength
            )  # Ensure minimum radius
            lon_radius = max(
                1.0, (lon_max_idx - lon_min_idx) / 2 * focus_strength
            )  # Ensure minimum radius
...
def demo_spatial_masking():
    ...
    if sweden:
        ...
        print(
            f"Sweden coverage: {binary_mask.sum() / binary_mask.numel() * 100:.2f}% of global grid"
        )
