...
def test_encoder_loading_only():
    ...
    try:
        ...
        print(
            f"✅ Architecture: {encoder.encoder.n_blocks} n_blocks -> {len(encoder.encoder.lgl_block.transformers)} transformers"
        )
        print(f"✅ Embedding dimension: {encoder.embed_dim}")
        print(
            f"✅ Expected formula: 2*{encoder.encoder.n_blocks}+1 = {2*encoder.encoder.n_blocks+1}"
        )
        print(f"✅ Actual transformers: {len(encoder.encoder.lgl_block.transformers)}")
        print(
            f"✅ Formula check: {2*encoder.encoder.n_blocks+1 == len(encoder.encoder.lgl_block.transformers)}"
        )

        print("\n📊 KEY VERIFICATION:")
        print("=" * 25)
        print(
            f"✅ If you see '🎯 Loaded 263/263 compatible weights (100% of encoder weights)' above"
        )
