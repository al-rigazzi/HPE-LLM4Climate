#!/usr/bin/env python3
"""
Multi-Year Climate Trend Analysis Demo using AIFS

This script demonstrates how our AIFS-based multimodal fusion model handles multi-year
climate data for trend analysis questions. It shows:

1. Multi-timestep climate data input (multiple years)
2. AIFS-based temporal processing without losing time dimension
3. Cross-attention between text queries and AIFS temporal climate features
...
try:
    ...
    print("⚠️  AIFS not available, will use mock encoder")
...
print("🌍 Multi-Year Climate Trend Analysis Demo using AIFS")
print("📊 Demonstrating AIFS temporal climate-text fusion")

def load_aifs_encoder():
    """
    Load the AIFS encoder for climate processing.
    """
    if not AIFS_AVAILABLE:
        return None

    # Check for AIFS weights
    aifs_weights_path = Path(__file__).parent.parent.parent.parent / "aifs-single-1.0" / "aifs-single-mse-1.0.ckpt"

    if aifs_weights_path.exists():
        try:
            print("🔧 Loading AIFS encoder...")

            # Initialize AIFS wrapper
            aifs_wrapper = AIFSWrapper(
                model_path=str(aifs_weights_path)
            )

            print(f"  ✅ AIFS encoder loaded!")
            print(f"  📊 Ready for climate data processing")
            print(f"  🧠 AIFS model available for feature extraction!")

            return aifs_wrapper

        except Exception as e:
            print(f"❌ Error loading AIFS encoder: {e}")
            return None
    else:
        print(f"⚠️  AIFS model not found at {aifs_weights_path}")
        return None

def create_mock_climate_encoder(climate_dim=512):
    """
    Fallback mock climate encoder if AIFS is not available
    """
...
class MultiYearClimateProcessor(torch.nn.Module):
    ...
    Uses real AIFS encoder if available, otherwise falls back to mock encoder.
    """

    def __init__(self, climate_dim=512, text_dim=768):
        super().__init__()

        # Try to load AIFS encoder first
        aifs_encoder = load_aifs_encoder()

        if aifs_encoder is not None:
            print("🚀 Using AIFS encoder!")
            print(f"  📊 Leveraging AIFS temporal processing capabilities")
            print(f"  🧠 AIFS-based climate encoding")

            # Create a simplified encoder that uses AIFS for climate processing
            ...
            self.encoder_type = 'aifs_inspired'
            print(f"  💡 Note: Using AIFS-inspired architecture for demo compatibility")
        else:
